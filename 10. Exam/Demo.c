...
void delete(Cashier *r){
    ...
    while (p != r){
        Cashier *delCashier = p;
        Customer *head = p->root_customer;
        while (head != NULL){
            Customer *delCustomer = head;
            head = head->next_customer;
            free(delCustomer);
        }
        p = p->next_cashier;
    ...
    while (r->root_customer != NULL){
        Customer *delRoot = r->root_customer;
        r->root_customer = r->root_customer->next_customer;
        free(delRoot);
    }
    free(r);
}
...
int main(){
    ...
    while (1){
        ...
        int x;
        scanf("%s", cmd);
        if(strcmp(cmd, "-1") == 0) rootCashier = enter(rootCashier, &cur, &numCustomer);
        else if(strcmp(cmd, "-2") == 0) advanced_enter(rootCashier, &numCustomer);
        ...
        else if(strcmp(cmd, "-6") == 0) countCustomerInAllLine(rootCashier);
